...
import { getMovieDetails, getMovieCredits, getSimilarMovies, getMovieVideos } from "../services/api";
  ...
  if (error || !movie) {
    return (
      <div className="text-center py-20 px-4">
        ...
        <Link to="/" className="mt-4 inline-block text-cyan-400 hover:underline">Go back home</Link>
      </div>
    );
  }
    ...
    <div className="space-y-12 animate-in fade-in duration-700">
          ...
          <div className="max-w-4xl">
            ...
            <div className="flex flex-wrap gap-4">
              {trailerKey && (
                ...
                >
                  <svg xmlns="http://www.w3.org/2000/svg" className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M8 5v14l11-7z"/></svg>
                  Watch Trailer
                </button>
              )}
              <div className="flex gap-2">
                {movie.genres?.map(genre => {
                  ...
                  const slug = projectGenre ? projectGenre.slug : genre.name.toLowerCase().replace(/\s+/g, '-');
                  return (
                    <Link 
                      key={genre.id} 
                      to={`/genre/${slug}`}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
        ...
        <section className="lg:col-span-2 space-y-6">
          <h2 className="text-2xl font-bold text-white border-l-4 border-cyan-500 pl-4">The Cast</h2>
          <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4">
            {cast.map(person => (
              <div key={person.id} className="group flex flex-col items-center">
                ...
                <h3 className="text-sm font-bold text-slate-100 text-center line-clamp-1">{person.name}</h3>
                <p className="text-xs text-slate-400 text-center line-clamp-1">{person.character}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <section className="space-y-8 bg-slate-900/50 p-6 rounded-3xl border border-slate-800 backdrop-blur-sm h-fit">
          ...
          <div className="space-y-4">
            <h3 className="text-sm font-bold uppercase tracking-wider text-slate-500">Release Date</h3>
            <p className="text-white font-medium">{movie.release_date}</p>
          </div>
          <div className="space-y-4">
            <h3 className="text-sm font-bold uppercase tracking-wider text-slate-500">Budget</h3>
            <p className="text-white font-medium">${movie.budget?.toLocaleString() || 'N/A'}</p>
          </div>
          <div className="space-y-4">
            <h3 className="text-sm font-bold uppercase tracking-wider text-slate-500">Revenue</h3>
            <p className="text-white font-medium">${movie.revenue?.toLocaleString() || 'N/A'}</p>
          </div>
          {movie.production_companies?.length > 0 && (
            <div className="space-y-4">
              <h3 className="text-sm font-bold uppercase tracking-wider text-slate-500">Production</h3>
      ...
      <section className="space-y-8 pb-12">
        <div className="flex items-center justify-between">
          <h2 className="text-2xl font-bold text-white border-l-4 border-cyan-500 pl-4">Similar Movies</h2>
        </div>
